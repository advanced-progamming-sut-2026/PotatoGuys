...
public class StartupHelper {
	 ...
	 *                  Please note that enabling this will block the thread
	 *                  until the child JVM process stops executing.
	 * @return whether a child JVM process was spawned or not.
	 */
	public static boolean startNewJvmIfRequired(boolean inheritIO) {
		...
		if (osName.contains("windows")) {
			...
			// By default, LWJGL3 extracts to the directory specified by
			// "java.io.tmpdir": usually, the user's home.
			// If the user's name has non-ASCII (or some non-alphanumeric)
			// characters in it, that would fail.
			// By extracting to the relevant "ProgramData" folder, which is
			// usually "C:\ProgramData", we avoid this.
			// We also temporarily change the "user.name" property to one
			// without any chars that would be invalid.
	...
	private static final String MAC_JRE_ERR_MSG = "A Java installation could not be found. "
		+ "If you are distributing this app with a bundled JRE, be sure to set "
	...
	private static final String LINUX_JRE_ERR_MSG = "A Java installation could not be found. "
		+ "If you are distributing this app with a bundled JRE, be sure to set "
		+ "the environment variable '__GL_THREADED_OPTIMIZATIONS' to '0'!";
	private static final String CHILD_LOOP_ERR_MSG = "The current JVM process is a spawned child "
		+ "JVM process, but StartupHelper has attempted to spawn another child JVM process! "
		+ "This is a broken state, and should not normally happen! Your game may crash "
	 ...
	 * @param inheritIO whether I/O should be inherited in the child JVM process.
	 *                 Please note that enabling this will block the thread until
	 *                 the child JVM process stops executing.
	 * @return whether a child JVM process was spawned or not.
	 */
	public static boolean startNewJvm0(boolean isMac, boolean inheritIO) {
		...
		// This state shouldn't usually be reachable, but this stops us from
