...
public class PlantFactory {
    ...
    public Plant create(PlantType type, int col, int lane, int level, boolean boosted, GameContext ctx) {
        ...
        // PlantAction feedAction = new ConfigFeedAction(null); // legacy, sheet-driven
        // Plant Food
        // return new Plant(sheet, buildAction(sheet), feedAction, col, lane, level,
        // boosted, ctx);
        return null;
    }
    ...
    // // Migrated to the config-driven path above; only reached when no
    // plant_actions.json entry exists yet.
    // PassiveAction.INSTANCE;
    // case LOBBER -> interval != null ? new LobberAction(interval) :
    // PassiveAction.INSTANCE;
    // case MELEE -> interval != null ? new MeleeAction(interval) :
    // PassiveAction.INSTANCE;
    // case HOMING -> interval != null ? new HomingAction(interval) :
    // PassiveAction.INSTANCE;
