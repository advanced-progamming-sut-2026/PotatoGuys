...
public class ExplosionAction extends PlantAction {
    ...
    public void update(Plant plant, GameContext ctx, float dt) {
        ...
        if (explosionDone && dealDamageDone)
            plant.dispose();
    }
    ...
    public FrameConfig draw(Plant plant, GameContext ctx) {
        Vector2 position = new Vector2(GameController.colToWorldX(plant.getCol()),
                GameController.laneToWorldY(plant.getLane()));
        Vector2 scale = new Vector2(0.65f, 0.65f);
        PamAnimationConfig pamAnimationConfig = plant.getSheet().pamAnimationConfig;
        return new FrameConfig(pamAnimationConfig.pamFilePath, config.explosionClip, stateTime, position, scale, null,
                false);
    }
