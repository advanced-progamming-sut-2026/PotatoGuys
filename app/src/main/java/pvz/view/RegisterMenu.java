...
public class RegisterMenu implements Menu {
    ...
    public Result handleInput(String input) {
        Matcher matcher;
        if ((matcher = RegisterMenuCommand.ENTER_MENU.getMatcher(input)) != null)
            return controller.enterMenu(matcher);
        if ((matcher = RegisterMenuCommand.REGISTER.getMatcher(input)) != null)
            return controller.register(matcher);
