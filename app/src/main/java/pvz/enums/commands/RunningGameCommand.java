...
public enum RunningGameCommand implements MenuCommand {
    ...
    PLANT("^plant\\s+plant\\s+-t\\s+(?<plantType>.+)\\s+-l\\s+" + 
        "\\(\\s*(?<plantX>\\d+)\\s*,\\s*(?<plantY>\\d+)\\s*\\)\\s*$"),
    PLANT_ZOMBIE(
            "^plant\\s+zombie\\s+-t\\s+(?<zombieType>.+)\\s+-l\\s+" + 
            "\\(\\s*(?<zombieX>\\d+)\\s*,\\s*(?<zombieY>\\d+)\\s*\\)\\s*$"),
    ...
    SHOW_TILE_STATUS("^show\\s+tile\\s+status\\s+-l\\s+" +
             "\\(\\s*(?<tileX>\\d+)\\s*,\\s*(?<tileY>\\d+)\\s*\\)\\s*$"),
    CHEAT_SPAWN_ZOMBIE(
            "^cheat\\s+spawn-zombie\\s+-t\\s+(?<zombieType>\\w+)\\s+-l" + 
            "\\s+\\(\\s*(?<zombieX>\\d+)\\s*,\\s*(?<zombieY>\\d+)\\s*\\)\\s*$"),
    HELP("^\\s*help\\s*$");

    private final String pattern;

    RunningGameCommand(String pattern) {
